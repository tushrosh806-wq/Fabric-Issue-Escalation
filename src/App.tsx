    ...
    try {
      ...
      setEscalations(prev => prev.map(item => 
        item.id === escalation.id 
          ? { ...item, status: 'Closed', resolution, resolved_at: resolvedAt } 
          : item
      ));
...
const NewEscalationModal = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Diagnostic: Check if we can actually see the profile
      ...
      // 2. Ensure profile exists and is synced
      const profileData = {
        id: user.id,
      ...
      if (profileSyncError) {
        ...
        if (profileSyncError.message.includes('permission denied') || profileSyncError.code === '42501') {
          throw new Error("Security Error: Row Level Security (RLS) is blocking the profile sync. Please run the 'RLS Fix' SQL provided in the instructions.");
        }
        throw new Error(`Profile sync failed: ${profileSyncError.message}`);
      }

      // 3. Final Verification: Can we see the profile now?
      ...
      if (insertError) {
        ...
        if (insertError.code === '23503') {
          throw new Error(`Foreign Key Error: ${insertError.message}. This usually means your user ID was not found in the 'profiles' table. Ensure RLS is disabled or has an 'INSERT' policy for the profiles table.`);
        }
...
const ReportsView = ({ escalations }: { escalations: Escalation[] }) => {
  const statusData = [
    { name: 'Open', value: escalations.filter(e => e.status === 'Open' || e.status === 'In Progress').length },
    { name: 'Closed', value: escalations.filter(e => e.status === 'Closed' || e.status === 'Resolved').length },
  ];
  ...
  const branchData = Object.entries(
    escalations.reduce((acc, e) => {
      const branchName = e.branch || 'Unknown';
...
export default function App() {
  ...
  const fetchEscalations = React.useCallback(async (silent = false) => {
  ...
  const stats = [
    ...
    { label: 'Pending', value: escalations.filter(e => e.status === 'Open' || e.status === 'In Progress').length, icon: AlertTriangle, color: 'bg-orange-50 text-orange-600' },
    { label: 'Closed', value: escalations.filter(e => e.status === 'Closed' || e.status === 'Resolved').length, icon: CheckCircle2, color: 'bg-green-50 text-green-600' },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F7F4] font-sans text-gray-900">
      ...
      <ResolutionModal
        ...
        onSuccess={() => fetchEscalations(true)}
